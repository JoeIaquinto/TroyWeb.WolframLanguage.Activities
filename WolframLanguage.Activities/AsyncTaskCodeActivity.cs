...
namespace WolframLanguage.Activities.Activities
{
	...
	public abstract class AsyncTaskCodeActivity<T> : AsyncCodeActivity, IDisposable
	{
		...
		protected override IAsyncResult BeginExecute(AsyncCodeActivityContext context, AsyncCallback callback, object state)
		{
			...
			var property = context.DataContext.GetProperties()[WolframLanguageScope.ParentContainerPropertyTag];
			var servicesApplication = property?.GetValue(context.DataContext);

			var taskCompletionSource = new TaskCompletionSource<T>(state);
			var task = ExecuteAsync(context, _cancellationTokenSource.Token, (servicesApplication ?? throw new ApplicationException()) as Application);
	...
	/// <summary>
	///     WolframLanguage AsyncTaskCodeActivity without result
	/// </summary>
